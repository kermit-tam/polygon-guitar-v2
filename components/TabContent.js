...
function extractNumericNotationComponents(line) {
  ...
  segments.forEach((segment, segIdx) => {
    ...
    while ((match = regex.exec(segment)) !== null) {
      ...
      if (beforeBracket) {
        // 檢查是否為和弦
        const chordMatch = beforeBracket.match(/[A-G][#b]?(?:maj|mj|m|min|dim|aug|sus|add|m7|7|9|11|13)?\d*(?:-\d+)*(?:sus\d*)?$/);
        if (chordMatch) {
...
const TabContent = ({ 
  ...
  const renderContent = () => {
    ...
    while (i < lines.length) {
      ...
      const strictChordPattern = /\b[A-G](#|b)?(maj|mj|m|min|sus|dim|aug)?(add|m7|maj7|7|9|11|13)?\d*((b|#)\d*)?(?:-\d+)*(sus\d*)?(\/[A-G][#b]?)?(?=\s|$|\||\b)/g;
      ...
      const isChordOnlyLine = validChordMatches.length > 0 && line.trim().split(/\s+/).every(part => {
        ...
        const chordWithSlash = part.match(/^[A-G][#b]?(maj|mj|m|min|sus|dim|aug)?(add|m7|maj7|7|9|11|13)?\d*((b|#)\d*)?(?:-\d+)*(sus\d*)?(\/[A-G][#b]?)?$/);
      ...
      const isEnglishWordsActuallyChords = englishWords.length > 0 && englishWords.every(word => 
        /^[A-G][#b]?(maj|mj|m|min|sus|dim|aug|add|m7|7|9|11|13)?\d*(?:-\d+)*(sus\d*)?$/i.test(word) || NC_PATTERN.test(word)
      );
      ...
      if (isLyric) {
        ...
        const prevHasChordPattern = prevLine && /\b[A-G][#b]?(maj|mj|m|min|sus|dim|aug|add|m7|7|9|11|13)?\d*((b|#)\d*)?(?:-\d+)*(sus\d*)?(?=\s|$|\||\b)/.test(prevLine);
      ...
      if (isNumericNotation) {
        ...
        const nextHasChordPattern = nextLine && /\b[A-G][#b]?(maj|mj|m|min|sus|dim|aug|add|m7|7|9|11|13)?\d*((b|#)\d*)?(?:-\d+)*(sus\d*)?(?=\s|$|\||\b)/.test(nextLine);
      ...
      if (isChord) {
        ...
        if (hasLyric) {
          ...
          const nextHasChordOnly = nextLine && /\b[A-G][#b]?(maj|mj|m|min|sus|dim|aug|add|m7|7|9|11|13)?\d*((b|#)\d*)?(?:-\d+)*(sus\d*)?(?=\s|$|\||\b)/.test(nextLine) && !/[\u4e00-\u9fff]/.test(nextLine);
  ...
  const ControlBar = () => {
    ...
    const chordStats = (() => {
      ...
      const chordPattern = /\b[A-G][#b]?(?:maj|mj|m|min|dim|aug|sus|add|m7|maj7|7|9|11|13)?\d*(?:-\d+)*(?:sus\d*)?(?:\/[A-G][#b]?)?(?=\s|$|\||\b)/g;
