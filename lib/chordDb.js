...
import { computeChordVoicing, computeAllVoicings } from './chordVoicingCalculator';
...
function parseChordName(chord) {
  ...
  if (!suffix && suffixPart) {
    const normalized = suffixPart.toLowerCase()
      ...
      .replace(/^m7$/, 'm7');
    if (guitarDb.suffixes.includes(normalized)) {
      suffix = normalized;
    } else {
      return null;
    }
  ...
  return { dbKey, suffix, bassNote, rootName: root };
}
...
export function getChordFromDb(chordName) {
  ...
  const { dbKey, suffix, bassNote, rootName } = parsed;
  ...
  if (bassNote) {
    ...
    /** 3) 用演算法計算 voicing（音樂理論推導指板位置） */
    const computed = computeChordVoicing(rootName, suffix, bassNote);
    if (computed) return buildChordResultFromPosition(computed, chordName);
    return null;
  }

  const chordEntry = chordList.find((c) => c.suffix === suffix);
  if (!chordEntry || !chordEntry.positions?.length) {
    const computed = computeChordVoicing(rootName, suffix, null);
    if (computed) return buildChordResultFromPosition(computed, chordName);
    return null;
  }
...
export function getChordAlternatives(chordName) {
  ...
  if (bassNote) {
    ...
    /** Algorithmic: compute multiple voicings at different positions */
    const computedPositions = computeAllVoicings(rootName, suffix, bassNote);
    if (computedPositions.length) {
      const list = computedPositions
        .map((p) => buildChordResultFromPosition(p, chordName))
        .filter(Boolean);
      if (list.length) {
  ...
  const chordEntry = chordList.find((c) => c.suffix === suffix);
  if (!chordEntry?.positions?.length) {
    const computedPositions = computeAllVoicings(rootName, suffix, null);
    if (computedPositions.length) {
      const list = computedPositions
        .map((p) => buildChordResultFromPosition(p, chordName))
        .filter(Boolean);
      if (list.length) return { list, defaultIndex: 0 };
    }
    return null;
  }
