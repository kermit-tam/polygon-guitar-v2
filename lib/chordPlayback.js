...
const STRUM_DELAY_FIRST = 0.25;  // 第一次撥弦：每弦 250ms
const STRUM_DELAY_REPEAT = 0.06; // 重複撥弦：每弦 60ms
const GAP_BETWEEN_STRUMS = 0.6;  // 兩次撥弦之間間隔約 0.6 秒

/**
 * 播放和弦（撥弦兩次，每音持續較長讓尾音自然 decay）
 */
export async function playChord(shape, duration = '1.2') {
  ...
  try {
    ...
    notes.forEach((note, i) => {
      sampler.triggerAttackRelease(note, duration, now + i * STRUM_DELAY_FIRST);
    });

    // 第二次撥弦（80ms 間隔），緊接第一次之後
    const repeatStart = now + (notes.length - 1) * STRUM_DELAY_FIRST + GAP_BETWEEN_STRUMS;
    notes.forEach((note, i) => {
      sampler.triggerAttackRelease(note, duration, repeatStart + i * STRUM_DELAY_REPEAT);
    });
  } catch (e) {
