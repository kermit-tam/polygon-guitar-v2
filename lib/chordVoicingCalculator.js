...
export function computeAllVoicings(rootName, quality, bassNoteName, maxResults = 6) {
  ...
  if (intervals.length === 3) {
    // Triad（大三／小三／減／屬七以外的三和弦等）：必須齊根、三、五／減五，避免 Cdim/D 只出現 C+Eb 而冇 Gb
    for (let i = 1; i < intervals.length; i++) {
      essentialPcs.add((rootPc + intervals[i]) % 12);
    }
  } else {
  ...
  for (let wStart = 0; wStart <= 9; wStart++) {
    ...
    const search = (sIdx, played) => {
      if (sIdx >= 6) {
        ...
        // 同一音高重複（例如低音 D 再喺鄰弦開弦 D）：偏好較乾淨嘅指法
        const pcCounts = new Map();
        for (let i = 0; i < 6; i++) {
          if (tmpPcs[i] >= 0) pcCounts.set(tmpPcs[i], (pcCounts.get(tmpPcs[i]) || 0) + 1);
        }
        for (const n of pcCounts.values()) {
          if (n > 1) score -= (n - 1) * 28;
        }
