...
/** Latest carousel uses same songIds as /playlist/最近上架 (manual slug/title or auto weekly). */
const LATEST_SECTION_SLUG = '最近上架'
const LATEST_SECTION_TITLE = '最近上架'

function pickPlaylistForLatestSection(autoList, manualList) {
  const manuals = manualList || []
  const autos = autoList || []
  const matchNamed = (p) =>
    p &&
    (String(p.slug || '').trim() === LATEST_SECTION_SLUG ||
      String(p.title || '').trim() === LATEST_SECTION_TITLE)
  const namedManual = manuals.find(matchNamed)
  if (namedManual?.songIds?.length) return namedManual
  const weekly = autos.find((p) => p.autoType === 'weekly')
  if (weekly?.songIds?.length) return weekly
  return null
}

function tabsFromPlaylistSongIds(playlist, tabsById, max) {
  if (!playlist?.songIds?.length || !tabsById) return []
...
export function buildHomeDataPayloadFromRaw({
  ...
  const latestSourcePl = pickPlaylistForLatestSection(autoPlaylistsData, manualPlaylistsData)
  let recentTabsData = tabsFromPlaylistSongIds(latestSourcePl, tabsById, 10)
  if (!recentTabsData.length) {
    recentTabsData = [...tabsArray]
...
export async function buildHomeDataPayload() {
  ...
  const latestSourcePl = pickPlaylistForLatestSection(autoPlaylistsData, manualPlaylistsData)
  let recentTabsData
  if (latestSourcePl?.songIds?.length) {
    const ids = latestSourcePl.songIds.slice(0, 10)
    recentTabsData = await getTabsByIds(ids)
    const orderMap = new Map(ids.map((id, i) => [id, i]))
    recentTabsData.sort((a, b) => (orderMap.get(a.id) ?? 999) - (orderMap.get(b.id) ?? 999))
  } else {
    recentTabsData = await getRecentTabsFromFirestore(10)
  }
