...
/** latestSongs 獨立 in-memory cache — 5 分鐘 TTL，直接查 Firestore，唔依賴 patch API */
let _latestSongsCache = null
let _latestSongsCacheTime = 0
const _LATEST_SONGS_TTL_MS = 5 * 60 * 1000

async function getFreshLatestSongs(count = 10) {
  if (_latestSongsCache && Date.now() - _latestSongsCacheTime < _LATEST_SONGS_TTL_MS) {
    return _latestSongsCache
  }
  try {
    const tabs = await getRecentTabsFromFirestore(count)
    _latestSongsCache = tabs
    _latestSongsCacheTime = Date.now()
    return tabs
  } catch (e) {
    console.warn('[homeData] getFreshLatestSongs failed:', e?.message)
    return null
  }
}
...
export function bustHomeDataApiCache() {
  _homeApiCache = null
  _homeApiCacheTime = 0
  _latestSongsCache = null
  _latestSongsCacheTime = 0
}

/** For /api/home-data: serve from in-memory when fresh, else getHomeData() and store. */
export async function getHomeDataCached() {
  // 45s in-memory cache hit — but still refresh latestSongs if its own 5min TTL expired
  if (_homeApiCache != null && Date.now() - _homeApiCacheTime < _HOME_API_CACHE_TTL_MS) {
    if (!_latestSongsCache || Date.now() - _latestSongsCacheTime >= _LATEST_SONGS_TTL_MS) {
      const freshTabs = await getFreshLatestSongs()
      if (freshTabs) {
        const photoMap = {}
        // re-use cached artistPhotoMap from the main payload if available, otherwise skip enrichment
        const slimmed = freshTabs.map((t) => {
          const cover = t.coverImage || t.albumImage ||
            (t.youtubeVideoId ? `https://img.youtube.com/vi/${t.youtubeVideoId}/mqdefault.jpg` : null) ||
            t.thumbnail || null
          return {
            id: t.id,
            title: t.title || '',
        ...
        _homeApiCache = { ..._homeApiCache, latestSongs: slimmed }
      }
    }
...
function slimPlaylistForHome(playlist) {
  ...
  return {
    ...
    ...(playlist.slug ? { slug: playlist.slug } : {})
  }
}
...
export async function buildHomeDataPayload() {
  ...
  const targetCount = Math.min(settings.hotTabs?.displayCount || 10, 100)
  let hotTabsData = defaultHotTabsData.slice(0, targetCount)
  if (settings.hotTabs?.useManual && settings.hotTabs?.manualSelection?.length > 0) {
    const manualIds = settings.hotTabs.manualSelection
      .map((t) => (typeof t === 'object' && t !== null ? t.id : t))
  ...
  if (snap?.exists()) {
    ...
    if (data && updatedAt && typeof updatedAt.toMillis === 'function') {
      ...
      if (ageMs <= CACHE_TTL_MS) {
        ...
        // 永遠直查最新 latestSongs（+10 reads），確保唔受 patch API 失敗影響
        try {
          const freshTabs = await getRecentTabsFromFirestore(10)
          if (freshTabs?.length > 0) {
            const freshLatest = freshTabs.map((t) => slimTabForHome(t, {}))
            return { ...data, latestSongs: freshLatest }
          }
        } catch (e) {
          console.warn('[homeData] fresh latestSongs query failed, using cached:', e?.message)
        }
        return data
      }
