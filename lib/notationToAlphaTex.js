...
/**
 * One tab note: fret.string, tie continuation `- . string`, or dead `x.string`.
...
function beatToTex(beat) {
  ...
  const effectParts = []
  if (beat.dotted) effectParts.push('d')
  if (chord) effectParts.push(`ch "${escapeChordForAlphaTex(chord)}"`)
  const effects = effectParts.length > 0 ? ` { ${effectParts.join(' ')} }` : ''
