/**
 * Node / 多數環境下 `URL.pathname` 會保留 % 編碼；Firestore `slug` 係 Unicode。
 * 解碼後先至同 `where('slug','==', …)` / `getTab` 一致。
 */
function decodeTabPathSegment(encoded) {
...
export function parsePolygonTabLink(url) {
  ...
  try {
    ...
    if (host === 'polygon.guitars' || host.endsWith('.polygon.guitars') || host === 'localhost' || host.startsWith('192.168.') || host.startsWith('127.0.0.1')) {
      return decodeTabPathSegment(pathMatch[1])
    }
    return null
  } catch {
    return null
