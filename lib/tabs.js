...
// Sanitize a slug to match generateTabSlug's rules — used as a recovery lookup
// for legacy URLs whose slug contained URL-unsafe chars (`/`, `(`, `)`, …).
function sanitizeSlugForLookup(s) {
...
// Tries: doc ID → slug → previousSlugs → sanitized slug.
// `.doc()` is wrapped in try/catch because Firestore throws when the path
// contains `/` (e.g., a legacy bad slug like `family-of-the-year-hero-(boyhood-/...`).
export async function getTab(idOrSlug, { skipCache = false } = {}) {
  ...
  // 1) Try as Firestore doc ID first (may throw for slugs containing `/`)
  try {
  ...
  // 3) Legacy slug stored in `previousSlugs` array (set by fix-bad-tab-slugs.js)
  try {
    const prevSnap = await getDocs(
      query(collection(db, TABS_COLLECTION), where('previousSlugs', 'array-contains', idOrSlug), limit(1))
    )
    if (!prevSnap.empty) {
  ...
  // 4) Sanitized slug lookup (handles raw URLs containing `/`, `()`, etc.)
  const sanitized = sanitizeSlugForLookup(idOrSlug)
  if (sanitized && sanitized !== idOrSlug) {
    try {
      const sanSnap = await getDocs(
        query(collection(db, TABS_COLLECTION), where('slug', '==', sanitized), limit(1))
      )
      if (!sanSnap.empty) {
        const d = sanSnap.docs[0]
        const result = { id: d.id, ...d.data() }
        setTabCache(idOrSlug, result)
        return result
      }
