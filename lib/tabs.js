...
// Falls back to getAllTabs() sort only if the dedicated cache is missing (first deploy or manual clear).
export async function getRecentTabs(count = 20) {
  ...
  try {
    ...
    if (!/permission|Permission|PERMISSION_DENIED/i.test(msg)) {
      console.warn('[getRecentTabs] latestSongs read failed, falling back to getAllTabs:', msg)
    }
  }

  // Fallback: derive from full allTabs cache (old behaviour, kept for first-deploy / cache miss)
