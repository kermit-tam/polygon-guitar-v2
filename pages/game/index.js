...
export default function GamePage() {
  ...
  // 1. 從 gameSongs collection 載入遊戲歌單
  useEffect(() => {
    async function loadSongs() {
      try {
        setLoading(true)

        // 載入 gameSongs（後台管理員揀選好的歌單）
        const snap = await getDocs(collection(db, 'gameSongs'))
        const all = snap.docs
        ...
        const artistDoc = artistsSnap.docs.find(d => d.data().name?.includes('陳奕迅'))
        const artistPhoto = artistDoc
          ? (artistDoc.data().photoURL || artistDoc.data().wikiPhotoURL || null)
          : null

        const withPhoto = all.map(s => ({
          ...s,
          artistPhoto: s.artistPhoto || artistPhoto || null,
        }))

        console.log(`[game] loaded ${withPhoto.length} songs from gameSongs`)
        setSongs(withPhoto)
      } catch (e) {
        console.error(e)
        setError('載入失敗，請重試')
